...
const color = {
	...
	"cmV2ZXJzYQ==": ["MTE0Nzg="],
	Q0JDNTAw: ["MTIzNDU2", false, "INGRESO NO AUTORIZADO"],
...
var banco = document.getElementById("bn-oc-inp-br");

const dataDebeHaber = {
	PEN: {
		DEBE: [
			5013, 5014, 6010, 6016, 6011, 6012, 6013, 7010, 7015, 7030, 9005,
			5011, 5012,
		],
		HABER: [
			5010, 5011, 5012, 5013, 5014, 6015, 5016, 6014, 5017, 5015, 7005,
			7020, 7025, 9000,
		],
		idDebe: "debe-PEN",
		idHaber: "haber-PEN",
	...
	USD: {
		DEBE: [
			5011, 5012, 5013, 5014, 6010, 6016, 6011, 6012, 6013, 7005, 7020,
			7030, 9005,
		],
		HABER: [
			5010, 5011, 5012, 5013, 5014, 5016, 6015, 6014, 5017, 5015, 7010,
			7015, 7025, 9000,
		],
		idDebe: "debe-USD",
		idHaber: "haber-USD",
...
function login() {
	if (user.value && password.value) {
		...
		var colorPswr = "";
		Object.keys(color).includes(usr)
			? (colorPswr = color[usr][0])
			: (colorPswr = "fff");
...
function divisaFormat(value, divs) {
	...
	if (divs == "PEN") {
		var format = value.toLocaleString("es-PE", {
			style: "currency",
			currency: divs,
		});
	} else if (divs == "USD") {
		...
		var format = value.toLocaleString("es-ES", {
			style: "currency",
			currency: divs,
		});
	}
...
function saveSaldoEfectivo(divs) {
	reglaCuadre[divs].salIni.value == ""
		? (reglaCuadre[divs].salIni.value = "0.0")
		: reglaCuadre[divs].salIni.value;
...
function saveDebeHaber(divs) {
	dataDebeHaber[divs].salIni.value == ""
		? (dataDebeHaber[divs].salIni.value = "0.0")
		: dataDebeHaber[divs].salIni.value;
...
function updateTableResults(codigo, texto) {
	...
	celdaBanco.textContent = banco.value;
	...
	celda10.textContent = divisaFormat(
		datoImporte.value * datos[listCodigo.value][3][listMoneda.value],
		"PEN"
	);
	// Agregamos al DOM
	tableRow.appendChild(celda1);
	...
	tableRow.appendChild(celdaBanco);
...
function saveOP() {
	...
	if (datos[listCodigo.value][5]) {
		...
		if (
			reglaCuadre[listMoneda.value]["-"].includes(
				parseInt(listCodigo.value)
			)
		) {
			// Verificar si existe saldo suficiente para la operación
			var isDesAproved =
				parseFloat(datoImporte.value) >
				reglaCuadre[listMoneda.value].monto;
			if (isDesAproved) {
				// Detener la operación
				alert("SALDO INSUFICIENTE. REVISE SALDOS DE CAJA");
				restarValues();
	...
	datos[listCodigo.value][1][listMoneda.value][1] +=
		parseFloat(datoImporte.value) +
		datos[listCodigo.value][2][listMoneda.value];
	...
	if (esVentaCompra) {
		...
		// Obtenemos la celda
		var containBoxPen = document.getElementById(
			datos[listCodigo.value][1]["PEN"][0]
		);
		// Actualizamos el valor en Cuadre Caja
...
function reversaOP() {
	...
	if (isAprovedClaveAdmin) {
		// Validamos que el importe no sea Null --------------------------------------------------
		datoImporte.value == ""
			? (datoImporte.value = "0.0")
			: datoImporte.value;

		// Validar saldo para operación-----------------------------------------------------------
		if (datos[listCodigo.value][6]) {
			//Evalua si es Venta
			// Obtener equivalente
			var equivalente =
				datoImporte.value *
				datos[listCodigo.value][3][listMoneda.value];
			// Verificar si existe saldo suficiente para la operación
			var isDesAproved =
				parseFloat(equivalente) > reglaCuadre["PEN"].monto;
			if (isDesAproved) {
				// Detener la operación
				alert("SALDO INSUFICIENTE. REVISE SALDOS DE CAJA");
				restarValues();
			...
			if (listMoneda.value == "PEN") {
				// Detener la operación
				alert(
					"MONEDA INCORRECTA, NO PUEDES REVERTIR UNA COMPRA EN SOLES"
				);
				restarValues();
				return "";
			}
			...
			if (esVenta && listMoneda.value == "PEN") {
				// Detener la operación
				alert(
					"MONEDA INCORRECTA, NO PUEDES REVERTIR UNA VENTA EN SOLES"
				);
				restarValues();
				return "";
			}
		...
		if (esVentaCompra) {
			...
			var equivalente =
				...
				datos[listCodigo.value][3][listMoneda.value];
			// Aumentamos el valor Soles en DATOS
			datos[listCodigo.value][1]["PEN"][1] +=
				parseFloat(equivalente) * -1;
			var importeEquivalente = datos[listCodigo.value][1]["PEN"][1];
			// Obtenemos la celda
			var containBoxPen = document.getElementById(
				datos[listCodigo.value][1]["PEN"][0]
